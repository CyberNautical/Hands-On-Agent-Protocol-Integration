...
Almost always **deployment name vs model name**.

`AZURE_OPENAI_DEPLOYMENT` must be the name *you* gave the deployment in the
Azure portal, which is often not the model name. If you deployed `gpt-4o` and
named the deployment `gpt-4o-prod`, the value is `gpt-4o-prod`.
